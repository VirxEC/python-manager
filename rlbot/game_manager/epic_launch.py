...
from rlbot.game_manager.preference import GameLauncherPreference, GameProcInfo
...
def launch_rocket_league(
    port: int, launcher_preference: GameLauncherPreference
) -> bool:
    """
    ...
    if CURRENT_OS != OS.WINDOWS:
        DEFAULT_LOGGER.warn(
            "We are not on Windows, launching Rocket League via Epic is unknown. "
            + f"Please launch Rocket League manually using the args {ideal_args} option to continue."
        )
        return False
    ...
    if launcher_preference.use_login_tricks:
        if launch_with_epic_login_trick(ideal_args, launcher_preference):
            ...
            DEFAULT_LOGGER.info("Epic login trick seems to have failed!")
    ...
    try:
        ...
        if epic_exe_path is not None:
            ...
            try:
                ...
                DEFAULT_LOGGER.info(f"Unable to launch via Epic due to: {e}")
    except:
        DEFAULT_LOGGER.debug("Unable to launch via Epic.")
...
def locate_epic_games_launcher_rocket_league_binary() -> Optional[Path]:
    ...
    try:
        ...
        DEFAULT_LOGGER.warn(
            "It looks like we're not on Windows, skipping Epic launcher search."
        )
        return
    ...
    def search_for_manifest_file(app_data_path: Path) -> Optional[dict[str, str]]:
    ...
    for possible_location in possible_registry_locations:
        try:
            ...
            path = (
                Path(
                    winreg.QueryValueEx(
                        winreg.OpenKey(possible_location[0], possible_location[1]),
                        "AppDataPath",
                    )[0]
                )
                / "Manifests"
        ...
        if binary_data is not None:
            path = (
                Path(binary_data["InstallLocation"]) / binary_data["LaunchExecutable"]
            )
            if path.is_file():
                return path
    ...
    if program_data is None:
        DEFAULT_LOGGER.warn("Can't find %programdata%, skipping Epic launcher search.")
        return

    path = Path(program_data) / "Epic" / "EpicGamesLauncher" / "Data" / "Manifests"
...
def get_auth_args_from_process() -> Optional[list[str]]:
...
def get_auth_args_from_logs() -> Optional[list[str]]:
...
def launch_with_epic_login_trick(
    ideal_args: list[str], launcher_preference: Optional[GameLauncherPreference] = None
) -> bool:
    """
